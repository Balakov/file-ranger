...
namespace Everything
{
    public static class SDK
    {
		...
		public static IEnumerable<SearchResult> Search(string text)
		{
			...
			for (uint i = 0; i < Everything_GetNumResults(); i++)
			{
                try
                {
                    Everything_GetResultDateModified(i, out long date);
                    Everything_GetResultSize(i, out long size);

                    string file = Marshal.PtrToStringUni(Everything_GetResultFileName(i));
                    string path = Marshal.PtrToStringUni(Everything_GetResultPath(i));
                    ...
                    searchResults.Add(new SearchResult()
                    {
                        ...
                        IsDirectory = Everything_IsFileResult(i)
                    });
                }
                catch { }
			}
